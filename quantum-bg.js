...
const cloud = Array.from({ length: CLOUD_COUNT }, () => ({
  x: 0,
  y: 0,
  vx: 0,
  vy: 0
...
function orbitalDensity(x, y, mode) {
  ...
  const decay = Math.exp(-r / 260); // smooth, large reach
  ...
  switch (mode) {
    ...
    case "p_x":
      return Math.abs(x / r) * decay;

    case "p_y":
      return Math.abs(y / r) * decay;

    case "d":
      return Math.abs((x * y) / (r * r)) * decay;
...
function drawElectronCloud() {
  cloud.forEach(p => {
    ...
    const flowX = -dy * 0.0001;
    const flowY = dx * 0.0001;
    ...
    // soft re-seeding far away (NO BOX)
